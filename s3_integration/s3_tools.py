...
class ProgressMonitor:
    ...
    def __call__(self, num: float):
        ...
        with self._lock:
            self.progress += num
            sys.stdout.write(
                f"S3 Progress: {round(self.progress/1024/1024,3)} of {round(self.size)}  MBs"
            )
...
class S3Connection:
    ...
    def _get_objects_from_bucket(self, bucket_name="", update_from_aws=True):
        ...
        if (
            ("Name", bucket_name) not in self.cached_bucket_objects.items()
        ) or update_from_aws:
            self.connect()
    ...
    def choose_bucket(self):
        ...
        choice = choice if choice > 0 and choice <= len(buckets_list) else None
    ...
    def put_file(self, filepath, s3_file_name_key):
        ...
        if self.active_bucket_name:
            with open(filepath, "rb") as data:
                self.s3Resource.Bucket(self.active_bucket_name).put_object(
    ...
    def upload_file(self, filepath, s3_file_name_key, metadata=None):
        ...
        if self.active_bucket_name:
            self.s3.upload_file(
                ...
                s3_file_name_key,
                ExtraArgs=metadata,
                Callback=ProgressMonitor(filepath),
                Config=None,
            )
        else:
            print(
                "Sorry, you need to activate a bucket first.  Afterwards, please redo the last attempt."
            )
            self.choose_bucket()
    ...
    def upload_fileobj(self, fileobj, s3_file_name_key="", metadata=None):
        ...
        if self.active_bucket_name:
            response = self.s3.upload_fileobj(
                ...
                s3_file_name_key,
                ExtraArgs=metadata,
                Callback=ProgressMonitor(fileobj),
                Config=None,
            )
        else:
            print(
